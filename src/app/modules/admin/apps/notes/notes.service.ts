...
export class NotesService
{
    ...
    {
        return this._httpClient.get<Label[]>('http://127.0.0.1:3000/api/apps/notes/labels').pipe(
            tap((response: Label[]) => {
    ...
    {
        return this._httpClient.post<Label[]>('http://127.0.0.1:3000/api/apps/notes/labels', {title}).pipe(
            tap((labels) => {
                // Update the labels
    ...
    {
        return this._httpClient.patch<Label[]>('http://127.0.0.1:3000/api/apps/notes/labels' , {label}).pipe(
            tap((labels) => {
     ...
     * @param id
     */
    deleteLabel(id: any): Observable<Label[]>
    {
        return this._httpClient.delete<Label[]>('http://127.0.0.1:3000/api/apps/notes/labels/' + id).pipe(
            tap((labels) => {
                // Update the notes
                this.getNotes().subscribe();
                // Update the labels
    ...
    {
        return this._httpClient.get<Note[]>('http://127.0.0.1:3000/api/apps/notes/all').pipe(
            tap((response: Note[]) => {
    ...
    {
        return this._httpClient.post<Note>('http://127.0.0.1:3000/api/apps/notes/tasks', {
            note,
            task
        }).pipe(switchMap(() => this.getNotes().pipe(
            switchMap(() => this.getNoteById(note._id))
        )));
    }
    ...
    {
        return this._httpClient.post<Note>('http://127.0.0.1:3000/api/apps/notes', {note}).pipe(
            switchMap(response => this.getNotes().pipe(
                switchMap(() => this.getNoteById(response._id).pipe(
                    map(() => response)
                ))
            )));
    }
    ...
    updateNote(note: Note): Observable<Note>
    {
        ...
        return this._httpClient.patch<Note>('http://127.0.0.1:3000/api/apps/notes', {updatedNote}).pipe(
            tap((response) => {
    ...
    {
        return this._httpClient.delete<boolean>('http://127.0.0.1:3000/api/apps/notes', {params: {id: note._id}}).pipe(
